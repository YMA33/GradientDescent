#ifndef ALGO_BGD_CUBLAS_GLM_H_
#define ALGO_BGD_CUBLAS_GLM_H_
...
public:
    ...
    BGD_CUBLAS_GLM(int num_tuples, int gradient_size, int num_classes, char* filename,
...
BGD_CUBLAS_GLM::BGD_CUBLAS_GLM(int n_tuples, int grad_size, int n_classes, char* fname,
...
void BGD_CUBLAS_GLM::compute_gradient_and_update_model(){
...
double BGD_CUBLAS_GLM::get_loss(){
	// row-major round-robin
    loss_kernel_round_robin<<<hypar_para->num_blocks, hypar_para->num_threads>>>(d_example, d_label, d_weight, d_loss, input_data->num_tuples, input_data->gradient_size, hypar_para->num_blocks * hypar_para->num_threads);	
    // 
    cudaDeviceSynchronize();
